...
class CascadeDISCO(nn.Module):
    def __init__(self, disco_path, device, adaptive, adaptive_iter):
        super().__init__()
...
def main():
    ...
    if args.debug:
        root = "log/adaptive_attack_results/debug"
    else:
        root = "log/adaptive_attack_results" 
    ...
    if args.model_name == "vgg16_bn":
        ...
        classifier = load_model(model_name=args.model_name, dataset='cifar10', threat_model='Linf').to('cuda')
    ...
    if args.attack == "autoattack":
        ...
        cascadedisco = CascadeDISCO(disco_path=args.disco_path, device=device, adaptive=args.adaptive, adaptive_iter=args.adaptive_iter)    
